...
- `Feature Space`：对采样到的中间层特征做稳定 3D PCA 投影，跟随左侧全局 epoch，支持区域预设、只看 FP/FN、epoch 轨迹播放、convex hull / density surface、点选查看原图位置、HTML 导出和分离指标。
...
- `Feature Space` 页面：稳定 3D PCA 特征空间投影、左侧全局 epoch 同步、区域预设、FP/FN 聚焦、轨迹播放、convex hull / density surface、点选定位、HTML 导出和分离指标。
